...
Stats::Stats(const string& theme) {
    // Simulate getting a dynamic at structure from Gemini API
    // json statResponse = Gemini().genStatsForTheme(theme);

    future<json> statstheme = Gemini().genStatsForTheme(theme);

    json stats = statstheme.get(); // Block until the result is ready
    ...
    for (const auto& attribute : stats["attributes"]) {
        for (const auto& [key, value] : attribute.items()) {
